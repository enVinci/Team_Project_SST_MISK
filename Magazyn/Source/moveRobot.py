...
class MoveRobot:
	...
	def goToDestinationPoint(self, destinationPoint): #zakładam, że jestem już obrócony, jeśli nie - dodam kilka linijek
		...
		returnCode, Angles = vrep.simxGetObjectOrientation(GlobalVar.vrepClientID, self.handle, -1,
													 vrepConst.simx_opmode_oneshot_wait)
		self.rotate((angle-Angles[2])*180/math.pi)
		dist = math.hypot(dx, dy)
		
		
		returnCode, rightAngle = vrep.simxGetJointPosition(GlobalVar.vrepClientID, self.right_motor_handle, vrepConst.simx_opmode_oneshot_wait)
		returnCode, leftAngle = vrep.simxGetJointPosition(GlobalVar.vrepClientID, self.left_motor_handle, vrepConst.simx_opmode_oneshot_wait)
		
		
		rightAngle += dist/self.R
		leftAngle += dist/self.R
		
		returnCode = vrep.simxSetJointTargetPosition(GlobalVar.vrepClientID, self.right_motor_handle, rightAngle, vrepConst.simx_opmode_oneshot_wait)
		returnCode = vrep.simxSetJointTargetPosition(GlobalVar.vrepClientID, self.left_motor_handle, leftAngle, vrepConst.simx_opmode_oneshot_wait)
	
	def rotate(self, degrees):
		#returnCode, Angles = vrep.simxGetObjectOrientation(GlobalVar.vrepClientID, self.handle, -1, vrep.simx_opmode_oneshot_wait)
	...
	def updatePosition(self):
		errorCode, PioneerPosition = vrep.simxGetObjectPosition(GlobalVar.vrepClientID, self.handle, -1, vrepConst.simx_opmode_oneshot_wait)
